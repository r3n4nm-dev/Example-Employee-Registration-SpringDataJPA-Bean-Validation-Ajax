...
public class DataTablesService {
	...
	private String[] colunas = { "funcionarioid", "nomeCompleto", "cpf", "funcao.nomeFuncao" };

	public Map<String, Object> execute(FuncionarioRepository funcionarioRepository, HttpServletRequest request) {

		// start = numero da primera linha da página (levando em consideração uma
		...
		// Definindo a busca
		String busca = buscarPor(request);

		// Definindo a paginação
		Pageable paginacao = PageRequest.of(paginaAtual, length, direcao, coluna);

		// Consulta ao banco de dados
		Page<Funcionario> pagina = queryBy(busca, funcionarioRepository, paginacao);
	...
	private String buscarPor(HttpServletRequest request) {
		String busca = request.getParameter("search[value]");
		if (busca.isEmpty()) {
			return "";
		}
		return busca;
...
// retorno da consulta
	private Page<Funcionario> queryBy(String busca, FuncionarioRepository funcionarioRepository, Pageable paginacao) {
		...
		String b = "%"+busca+"%"; 	// "like" SQL (repository)
		return funcionarioRepository.findByNomeCompletoLikeOrCpfLikeOrFuncaoNomeFuncaoLike(b, b, b, paginacao);
	}
